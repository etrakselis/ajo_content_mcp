...
const outPath = process.argv[3] ?? '.env.credentials';

if (!jsonPath) {
  console.error('Usage: node scripts/load-credentials-env.mjs <credentials-json-file> [output-env-file]');
  process.exit(1);
}
...
for (const [key, value] of Object.entries(env)) {
  if (!value) continue;
  lines.push(`if [ -z "\${${key}:-}" ]; then export ${key}="${escape(value)}"; fi`);
}
